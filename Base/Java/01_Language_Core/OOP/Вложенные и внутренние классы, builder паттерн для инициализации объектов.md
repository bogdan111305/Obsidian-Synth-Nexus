...
> 4 типа вложенных классов: **Static nested** (нет ссылки на внешний), **Inner** (нестатический, хранит `this$0` → утечка памяти!), **Local** (внутри метода), **Anonymous** (анонимный). Builder — решение для объектов со многими параметрами вместо телескопических конструкторов.
> На интервью: `this$0` ссылка в inner class, почему анонимный класс захватывает effectively final, Builder vs record, когда static nested vs inner.
