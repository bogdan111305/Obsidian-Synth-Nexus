...
> На интервью: type erasure + bridge methods, heap pollution, Wildcard Capture, Super Type Tokens (TypeReference), unbounded wildcard `<?>` vs raw type.
...
- Что такое type erasure? Что происходит с `List<String>` в байткоде?
- Чем `List<?>` отличается от `List<Object>` и от raw `List`?
- Что такое PECS? Как выбрать между `<? extends T>` и `<? super T>`?
- Почему нельзя `new T()`, `new T[]`, `instanceof List<String>`?
- Что такое Heap Pollution? Когда `@SafeVarargs` применим, а когда нет?
- Что такое bridge method? При каких условиях компилятор его генерирует?
- Почему `void process(List<String>)` и `void process(List<Integer>)` не могут сосуществовать?
- Что такое Wildcard Capture? Почему `list.set(0, list.get(0))` не компилируется для `List<?>`?
- Что такое Super Type Token? Как Jackson знает `List<String>` vs `List<Integer>`?
- Что такое reifiable type? Почему `List<String>[]` не компилируется?
- Чем `<T extends Number & Comparable<T>>` отличается от `<T extends Comparable<T> & Number>`?

## Подводные камни

- **Raw type в коде** — `List list = ...` отключает все type checks, включая для всего класса. Компилятор выдаёт только warning.
- **Перегрузка после erasure** — `process(List<String>)` и `process(List<Integer>)` — compile error: erasure обеих одинакова. Это не очевидно на первый взгляд.
- **Heap pollution через varargs** — `<T> T[] toArray(T... args)` создаёт `Object[]`, не `T[]`. Присвоение `String[] s = toArray("a", "b")` → `ClassCastException` в runtime без явной ошибки компиляции.
- **`@SafeVarargs` — контракт, не гарантия** — если нарушить контракт (сохранить ссылку на массив varargs), runtime ClassCastException всё равно случится, только без предупреждения.
- **`new TypeReference<X>() {}` создаёт класс** — каждый вызов создаёт новый анонимный класс в Metaspace. Кэшируй в `static final`.
- **`List<Object>` ≠ `List<String>` superset** — нельзя передать `List<String>` туда, где ожидается `List<Object>`. Invariance generics — не массивы.
- **Bridge method в Reflection** — `method.isBridge()` нужно проверять при итерации по методам через Reflection, иначе bridge methods будут обработаны дважды.
- **Bounded type: класс первым** — `<T extends Number & Comparable<T>>` — порядок: класс → интерфейсы. `<T extends Comparable<T> & Number>` — compile error если `Number` не интерфейс (он класс).