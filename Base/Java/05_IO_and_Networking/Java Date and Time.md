...
> [!WARNING] Устаревший API — не используй в новом коде
> `java.util.Date` и `java.util.Calendar` мутабельны и не thread-safe. `SimpleDateFormat` не thread-safe. Используй `java.time` для любого нового кода.
