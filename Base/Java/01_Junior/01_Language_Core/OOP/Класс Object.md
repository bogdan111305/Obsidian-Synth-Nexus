...
---
Класс `Object` — это «Адам» мира Java. Это корень всей иерархии, и каждый класс, который вы пишете, неявно наследует его гены.
...
Обратная крайность тоже возможна и формально не нарушает контракт: если `hashCode()` всегда возвращает одну и ту же константу (например, `1`), консистентность соблюдена, но все объекты оказываются в одной корзине. `HashMap` вырождается в связный список, и операции, которые должны быть `O(1)` (`get`, `put`, `containsKey`), становятся `O(n)`, потому что внутри корзины элементы приходится сравнивать через `equals()` один за другим.
...
---
## wait, notify, notifyAll: Низкоуровневая магия
...
**Почему эти методы объявлены в `Object`, а не в `Thread`?** Потому что блокируется и ожидает не поток как сущность, а монитор конкретного объекта — любой объект в Java может выступать точкой синхронизации, и вокруг одного и того же монитора может ожидать сразу несколько потоков. Раз точкой синхронизации является объект, то и API ожидания/пробуждения логично прикреплён именно к `Object`.
...
`getClass()` объявлен в `Object` как `final` — переопределить его нельзя. Это гарантирует, что метод всегда возвращает настоящий рантайм-класс объекта, а не что-то, подменённое логикой подкласса, и на это можно полагаться, например, в собственной реализации `equals()`, где часто используется именно строгое сравнение классов вместо `instanceof`.

---
## Object Header: Анатомия в памяти
...
---
## Смерть finalize() и Cleaner API

Метод `finalize()` официально признан «ошибкой молодости»: в Java 18 (JEP 421) он объявлен deprecated for removal, но физически ещё не удалён — финализация по-прежнему включена по умолчанию, удаление запланировано на одну из будущих версий.
