...
По той же причине не стоит ловить `Throwable` целиком. `catch (Throwable t)` перехватывает не только `Exception`, но и `Error` — например, `OutOfMemoryError` или `StackOverflowError`. Такой перехват маскирует неисправимое состояние JVM: программа продолжает выполняться в момент, когда единственно правильное поведение — упасть (fail-fast) и дать окружению (supervisor, k8s) перезапустить процесс. Ловить `Throwable` оправдано только в узких инфраструктурных точках — например, top-level обработчик потока, который логирует ошибку перед тем, как дать потоку/процессу завершиться, — но не как общий паттерн обработки ошибок в бизнес-коде.
...
Иерархия проектируется так намеренно, а не произвольно. `ErrorCode` даёт клиенту API машиночитаемый код ошибки, не зависящий от текста сообщения (текст можно свободно менять для читаемости логов, код — это контракт, на который клиент может опираться в своей логике). `context` — неизменяемая карта (`Map.copyOf`) с деталями инцидента для логов/клиента, не раскрывающая внутреннее состояние сервиса (стек, имена классов, SQL). Два конструктора соответствуют двум происхождениям ошибки: первый — для нарушения бизнес-правила, обнаруженного самим доменным кодом (нет `cause`, ошибка не техническая, а логическая); второй — для оборачивания технического исключения (JDBC, сеть, внешний API) с сохранением цепочки через `cause`, чтобы не терять исходный стек (см. Exception Chaining выше).

**Перехват в Spring — `@RestControllerAdvice`:**

`@RestControllerAdvice` — глобальный, сквозной (cross-cutting) обработчик: Spring перехватывает исключение, вылетевшее из метода любого `@RestController` приложения, и ищет среди всех `@ExceptionHandler`-методов такой, чей параметр — ближайший по иерархии наследования к брошенному исключению (тот же принцип, что и порядок `catch`: от частного к общему типу). Это убирает дублирование `try-catch` в каждом контроллере — маппинг «тип исключения → HTTP-ответ» описан в одном централизованном месте.
