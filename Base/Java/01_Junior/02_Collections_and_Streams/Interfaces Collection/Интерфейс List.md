...
**Почему рост именно ×1.5, а не ×2 и не +константа:** фиксированный прирост (например, +10) даёт O(n) resize'ов на n добавлений → квадратичная сложность вставки. Кратный рост (×1.5 или ×2) даёт амортизированный O(1) — количество resize'ов растёт логарифмически. ×1.5 выбран как компромисс: ×2 быстрее сокращает число resize'ов, но оставляет больше неиспользуемой памяти в среднем и (в некоторых аллокаторах) не даёт переиспользовать память старых массивов под новый, потому что новый всегда больше суммы всех предыдущих.
...
`get(index)` внутри сравнивает `index` с `size / 2`: если `index < size / 2`, обход идёт от `first` вперёд, иначе от `last` назад — это вдвое сокращает худший случай, но не меняет порядок сложности (всё равно O(n)).
...
**Сортировка по нескольким критериям** — через цепочку `Comparator.thenComparing()`: первый компаратор сравнивает по основному полю, а `thenComparing` применяется только когда предыдущий вернул 0 (поля равны), то есть работает как "затем сортировать по...":
...
// Сначала по фамилии, при равенстве фамилий — по имени, при равенстве обоих — по возрасту
```
