...
- **Неизменяемость** — любая операция (`substring`, `replace`, `toUpperCase`...) возвращает новый объект; внутренний массив помечен `final`. Как следствие — `String` потокобезопасен без какой-либо синхронизации, и безопасно раздавать одну и ту же ссылку в разные места кода: никто не сможет незаметно изменить содержимое для остальных держателей ссылки. Именно это техническое свойство и делает возможным String Pool ниже — экономия памяти на разделяемых объектах работает только потому, что их нельзя испортить чужой мутацией.
- **Пул строк (String Pool)** — строковые литералы (`"text"`) автоматически интернируются при загрузке класса и кладутся в пул; физически это `StringTable` — внутренняя хеш-таблица HotSpot (до Java 7 жила в PermGen, с Java 7 — в обычном heap, что позволило GC собирать неиспользуемые записи пула как обычные объекты). `new String("text")` создаёт отдельный объект в куче, минуя пул; `intern()` возвращает (или добавляет и возвращает) ссылку из пула.
...
- **Кэширование `hashCode()`** — вычисляется лениво при первом вызове и кэшируется в приватном поле `hash` (с Java 13 — ещё и `boolean hashIsZero`, чтобы отличать «ещё не вычисляли» от «вычислили и получили 0», JDK-8221836). Первый вызов — O(n) по длине строки, все следующие — O(1), просто чтение поля. Это важно для `HashMap`/`HashSet`: каждый `put()`/`get()` со строковым ключом вызывает `hashCode()`, и для часто переиспользуемых ключей (заголовки HTTP, имена полей, коды) кэш даёт заметный выигрыш на lookup. Подводный камень: после `intern()` кэш хэша не переносится на интернированную строку из пула — если это другой объект, он пересчитает `hashCode()` при первом обращении к нему.
- **Конкатенация `+`/`concat()` в цикле — антипаттерн**: каждая итерация создаёт промежуточный объект. Для многократной сборки строки используй `StringBuilder` (см. §2). Начиная с Java 9 сама операция `+` вне цикла компилируется иначе — не в цепочку `StringBuilder.append()`, а в вызов `invokedynamic` (механика разобрана в Senior Insights ниже) — но это не отменяет антипаттерн: внутри цикла на каждой итерации всё равно создаётся новый результат.
...
- **Как устроен буфер** — внутри `StringBuilder` держит массив (как и `String`, `char[]` в Java 8 / `byte[]` с CODER-флагом в Java 9+) и счётчик текущей длины `count`. `append()` дописывает новые данные начиная с индекса `count` и сдвигает счётчик; сам символьный массив не пересоздаётся, пока хватает места. Конструктор без аргументов резервирует ёмкость 16 символов по умолчанию.
- **Рост буфера** — когда новых данных больше не помещается в оставшуюся свободную часть массива, выделяется новый массив большего размера, и старые данные копируются в него (`Arrays.copyOf`); формула роста — `newCapacity = oldCapacity * 2 + 2` (аналог механики `ArrayList`, но агрессивнее). Каждый такой resize — это лишняя аллокация и полное копирование накопленных данных, поэтому если конечный размер известен заранее, `new StringBuilder(expectedSize)` избавляет от лишних resize.
...
- **`Pattern`** компилирует regex один раз (компиляция дорогая) — храни в `static final` поле, а не создавай на каждый вызов. `Pattern` иммутабелен и потокобезопасен — один и тот же скомпилированный объект можно свободно шарить между потоками.
- **`Matcher`** выполняет поиск/сопоставление по скомпилированному `Pattern` и хранит состояние текущего поиска (позицию, найденные группы) — поэтому он, в отличие от `Pattern`, не thread-safe: на каждый поток/вызов нужен свой `Matcher` (`pattern.matcher(text)` — дешёвая операция, в отличие от компиляции самого `Pattern`). Это то же разделение «неизменяемая конфигурация / изменяемое состояние», что у `StringBuilder`/`StringBuffer`.
...
Не путай String Deduplication со String Pool/`intern()` (см. ниже) — это разные механизмы на разных уровнях. `intern()` — явный запрос программиста вернуть каноническую ссылку из глобального пула: два ранее разных объекта `String` схлопываются в один и тот же объект с одним `byte[]`. String Deduplication — фоновая работа GC: она объединяет только внутренние `byte[]`-массивы у строк с одинаковым содержимым, а сами объекты `String` остаются разными (`s1 == s2` по-прежнему `false`, `s1.equals(s2)` — `true`). Deduplication не требует правок кода, работает только на G1, не уменьшает число объектов `String` (только количество дублирующихся массивов) и добавляет фоновый overhead самому GC (ориентировочно 5–10%, зависит от workload).
...
### Конкатенация строк и `invokedynamic` (Java 9+)

В Java 8 выражение `"Hello " + name + "!"` компилируется в цепочку `new StringBuilder().append("Hello ").append(name).append("!").toString()`. Проблема этого подхода: `StringBuilder` не знает заранее финальный размер результата, поэтому по ходу `append()` возможен как минимум один resize с копированием внутреннего массива (см. §2 — тот же механизм роста буфера).

Начиная с Java 9 (JEP 280) компилятор вместо этого генерирует вызов `invokedynamic`, привязанный к bootstrap-методу `StringConcatFactory.makeConcatWithConstants()`. Он один раз на call site анализирует шаблон конкатенации и строит специализированный код, который: (1) заранее вычисляет точный размер результирующего массива; (2) аллоцирует `byte[]` нужного размера один раз; (3) копирует части сразу в него, без промежуточного `StringBuilder` и без resize. Для LATIN1-строк (см. Compact Strings выше) дополнительно используется SIMD-векторизация. Величина ускорения зависит от конкретного шаблона и прогрева JIT — не гарантированный процент, а общее направление оптимизации. Важно: это ускоряет одиночное выражение конкатенации, а не спасает от антипаттерна конкатенации в цикле (§1) — там на каждой итерации всё равно создаётся новый результат.
