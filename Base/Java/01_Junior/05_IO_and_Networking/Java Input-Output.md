...
|`WatchService`|Подписка на события файловой системы (создание/изменение/удаление) без ручного опроса.|Отслеживание изменений в директории (§6).|
...
Сериализация — преобразование объекта в поток байтов; десериализация — восстановление объекта из потока. Классы: `ObjectOutputStream`/`ObjectInputStream`, объект должен реализовывать `Serializable`. `Serializable` — маркерный интерфейс: у него нет ни одного метода, он лишь помечает класс как разрешённый для сериализации; саму механику (обход полей через reflection, запись их значений) выполняет `ObjectOutputStream`, а не реализующий класс. Поле `transient` в сериализацию не попадает (при десериализации получает значение по умолчанию: `0`/`null`/`false`).

`serialVersionUID` — идентификатор версии класса для сериализации. При десериализации JVM сравнивает `serialVersionUID`, записанный в потоке байтов, с тем, что объявлен в классе на данный момент; при несовпадении бросается `InvalidClassException`. Если не указать его явно, компилятор сгенерирует значение автоматически на основе структуры класса (полей, методов, интерфейсов) — из-за этого любое, даже безобидное на первый взгляд изменение класса (добавление метода, смена модификатора) может сломать совместимость со старыми сериализованными данными. Указывай `serialVersionUID` вручную, чтобы явно контролировать, какие изменения класса считать совместимыми.
...
Современный API для файлов и директорий: `Path` — путь, `Files` — утилитные операции над ним. `Path` — абстракция над путём в файловой системе вообще, а не только в локальной: тот же API работает с любым провайдером `FileSystem` (например, путь внутри ZIP-архива через `FileSystems.newFileSystem(...)`), в отличие от старого `java.io.File`, жёстко привязанного к локальной ФС. Именно поэтому `Files`-методы принимают `Path`, а не `File`.
...
**`WatchService`** — API для подписки на события файловой системы (создание/изменение/удаление файла) без ручного опроса (polling) директории в цикле. Директория регистрируется в сервисе через `path.register(watchService, StandardWatchEventKinds.ENTRY_CREATE, ENTRY_MODIFY, ENTRY_DELETE)`, после чего `watchService.take()` блокируется до появления события и возвращает `WatchKey` со списком случившихся событий. Используется там, где приложению нужно реагировать на изменения файлов "снаружи" (конфиги, папки с входящими файлами), не тратя CPU на постоянный опрос.
...
`java.nio` — `Channel` + `Buffer` для высокопроизводительного I/O. `FileChannel` — канал для чтения/записи файлов, эффективен на больших файлах.

**Как устроен `Buffer`** — любой буфер (`ByteBuffer` и т.п.) держит три указателя поверх внутреннего массива: `capacity` (общий размер, фиксирован на всё время жизни буфера), `position` (текущее место записи/чтения) и `limit` (граница, дальше которой читать/писать нельзя). В режиме записи `put()` дописывает данные и сдвигает `position` вперёд, пока не достигнет `limit` (изначально равного `capacity`). `flip()` переключает буфер из режима записи в режим чтения: запоминает, сколько реально записано (`limit = position`), и сбрасывает `position = 0` — именно поэтому `flip()` обязателен перед чтением того, что только что записали. После чтения `clear()` возвращает буфер в режим записи (`position = 0`, `limit = capacity`; старые данные физически остаются в массиве, но становятся логически недоступны), а `compact()` делает то же самое, но сохраняет непрочитанный хвост, сдвигая его в начало буфера — полезно при частичном чтении.
...
**Неблокирующий режим и `Selector`** — не все каналы одинаковы: `configureBlocking(false)` умеют сетевые каналы (`SocketChannel`, `ServerSocketChannel`), а `FileChannel` всегда блокирующий и non-blocking режим не поддерживает — для файлов преимущество неблокирующего I/O не работает так же, как для сокетов. `Selector` — механизм мультиплексирования для неблокирующих каналов: один поток может одновременно отслеживать события готовности (`OP_READ`, `OP_WRITE`, `OP_ACCEPT`, `OP_CONNECT`) у множества `SocketChannel`, зарегистрированных через `channel.register(selector, SelectionKey.OP_READ)`. Вызов `selector.select()` блокируется, пока хотя бы один из зарегистрированных каналов не станет готов к операции, после чего возвращает набор `SelectionKey` для обработки. Это и есть основа масштабируемых серверов: один поток способен обслуживать тысячи соединений вместо модели «поток на соединение», характерной для блокирующего `java.io` — на этом принципе построены Netty, NIO-коннектор Tomcat и подобные им.
