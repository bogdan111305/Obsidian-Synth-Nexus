...
`ZoneId` и `ZoneOffset` — не одно и то же, и `ZonedDateTime` неспроста хранит оба. `ZoneId` — регион (`"Europe/Paris"`), несущий с собой полный набор правил перехода на летнее/зимнее время (DST) для всех дат прошлого и будущего. `ZoneOffset` — просто фиксированное смещение от UTC (`+02:00`) в конкретный момент, без знания о том, что оно может измениться. `ZonedDateTime` хранит `ZoneId` (чтобы знать правила и уметь пересчитать смещение на любую дату) и текущий, уже разрешённый по этим правилам `ZoneOffset` — для одной и той же зоны `Europe/Paris` он будет разным зимой (`+01:00`) и летом (`+02:00`).
...
Через `Instant` — единственную точку соприкосновения обоих API: внутри `Date` тоже хранит миллисекунды от эпохи Unix, то есть в точности то же представление, что и `Instant`, без какой-либо привязки к календарю или часовому поясу. Поэтому переход всегда идёт именно через `Instant`, а не напрямую в `LocalDateTime` — у `Date` просто нет достаточной информации, чтобы однозначно превратиться в дату/время конкретного региона без явного указания зоны. Всегда указывай `ZoneId` явно, иначе конвертация неявно зависит от системного часового пояса.
...
- **Не игнорируй переход на летнее время** при арифметике над `ZonedDateTime` — прибавление часов может привести к неочевидному сдвигу даты. Конкретно: `zdt.plusHours(24)` и `zdt.plusDays(1)`/`Period.ofDays(1)` не всегда дают одинаковый результат в дни перехода на летнее/зимнее время. `plusHours()` (как и вся арифметика `Duration`) считает точное количество часов на временной шкале — в сутках с переходом это может оказаться 23 или 25 часов, а не 24. `plusDays()`/`Period` — календарная операция: она сдвигает именно дату, независимо от того, сколько фактических часов между ними прошло. Выбор между ними зависит от смысла операции: "ровно через 24 часа" (мониторинг, таймеры) — `Duration`/`plusHours`; "на следующий день в то же время" (напоминания, расписания) — `Period`/`plusDays`.
