> [!QUOTE] Суть
> `wait()`, `notify()`, `notifyAll()` — механизм кооперации через монитор объекта. `wait()` освобождает монитор и помещает поток в wait set. `notify()` будит один случайный поток. Все три должны вызываться **только внутри `synchronized` блока** (иначе `IllegalMonitorStateException`).

> [!WARNING] Ловушка: spurious wakeup
> Поток может проснуться из `wait()` без вызова `notify()` (spurious wakeup). Всегда оборачивай `wait()` в `while (!condition)`, а НЕ в `if (condition)`. Иначе поток продолжит без выполнения условия.

## Основы

Методы `wait()`, `notify()`, и `notifyAll()` из класса `Object` предназначены для **кооперативного взаимодействия потоков** через **монитор** объекта. Они позволяют одному потоку ожидать выполнения условия, а другому — уведомлять о его наступлении.
