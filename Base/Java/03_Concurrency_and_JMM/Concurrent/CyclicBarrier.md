...
> [!QUOTE] Суть
> **CyclicBarrier** — N потоков ждут друг друга в барьере. Когда все N вызвали `await()` — все разблокируются и барьер **сбрасывается** (в отличие от CountDownLatch). Опционально: выполняет `Runnable` при достижении барьера. Для многоэтапных параллельных вычислений.

`CyclicBarrier` — это синхронизатор из пакета `java.util.concurrent`, который позволяет группе потоков (участников) ожидать друг друга в определённой точке (барьере), прежде чем продолжить выполнение. Его ключевая особенность — **цикличность**: после срабатывания барьера он автоматически сбрасывается и готов к повторному использованию.
