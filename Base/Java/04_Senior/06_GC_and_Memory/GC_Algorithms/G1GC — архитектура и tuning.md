...
- G1 Full GC — с JDK 10 (JEP 307) параллельный mark-sweep-compact (столько потоков, сколько задано `ParallelGCThreads`), но всё равно полная STW-пауза и очень дорогая (~секунды на большом heap). Его появление = что-то сломано
