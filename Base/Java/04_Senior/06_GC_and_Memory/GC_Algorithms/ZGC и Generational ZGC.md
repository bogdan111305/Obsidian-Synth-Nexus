...
Это даёт:
- В общем случае — умеренный прирост throughput (в бенчмарках JEP 439 порядка ~10% над non-gen ZGC); на specific workloads с большим количеством короткоживущих объектов (напр. Cassandra-бенчмарк с heap в 4 раза меньше) заявлялся прирост throughput в разы — это не универсальная цифра, а частный случай
