...
> [!WARNING] Не задал `serialVersionUID` — рискуешь потерять данные
> Если не задать явно — JVM генерирует UID из структуры класса (имён полей, методов, сигнатур). При любом изменении класса UID меняется → старые сериализованные данные несовместимы → `InvalidClassException` при десериализации.
