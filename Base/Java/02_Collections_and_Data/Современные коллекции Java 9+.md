---
title: "Java — Современные коллекции Java 9+"
tags: [java, collections, modern-java, immutable, java9, java21, sequenced-collections]
updated: 2026-03-04
---
...
> [!QUOTE] Суть
> `List.of()`, `Set.of()`, `Map.of()` (Java 9+) — создают **неизменяемые** коллекции (UnsupportedOperationException при попытке изменить). `Set.of()` не гарантирует порядок. `SequencedCollection` (Java 21) — унифицированный доступ к первому/последнему элементу.

> [!WARNING] Ловушка: List.of() vs Arrays.asList()
> `Arrays.asList()` возвращает список **фиксированного размера** (нельзя add/remove, но можно set). `List.of()` — полностью иммутабельный (нельзя даже set). Не передавай `List.of()` куда ожидается изменяемый список.

Java 9+ добавил фабричные методы `List.of()`, `Set.of()`, `Map.of()` для создания иммутабельных коллекций, `copyOf()` для защищённых копий, а Java 21 — `SequencedCollection` для унифицированного доступа к первому/последнему элементу.
