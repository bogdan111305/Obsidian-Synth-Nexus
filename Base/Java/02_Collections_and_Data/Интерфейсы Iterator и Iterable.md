...
> [!QUOTE] Суть
> `Iterable` = умеет возвращать `Iterator`. `Iterator` = имеет `hasNext()`/`next()`/`remove()`. `for-each` компилируется в `Iterator`. **Нельзя** модифицировать коллекцию во время итерации через `for-each` — только через `iterator.remove()`.

> [!WARNING] Ловушка: ConcurrentModificationException
> Изменение коллекции во время итерации `for-each` (добавление/удаление элементов) бросает `ConcurrentModificationException`. Используй `Iterator.remove()` или `removeIf()` для безопасного удаления.
