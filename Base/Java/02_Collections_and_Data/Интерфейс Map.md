...
> [!QUOTE] Суть
> **Map** — хранит уникальные ключи → значения. `HashMap`: O(1) get/put (amortized), load factor 0.75, buckets + чейнинг → TreeNode при >8 элементов в bucket (Java 8+). `TreeMap`: O(log n), красно-чёрное дерево, отсортирован. `LinkedHashMap`: порядок вставки/доступа.

> [!WARNING] Ловушка: HashMap и mutable ключи
> Если использовать изменяемый объект как ключ HashMap и изменить его после `put()`, `hashCode()` изменится и объект **потеряется** в Map. Ключи должны быть иммутабельными (String, Integer, record).

`Map` хранит пары **ключ-значение** с уникальными ключами. Реализации: `HashMap` (O(1) amortized), `LinkedHashMap` (порядок вставки), `TreeMap` (O(log n), сортировка), `ConcurrentHashMap` (многопоточность), `WeakHashMap` (GC-friendly ключи).
...
**Как работает `put(key, value)` шаг за шагом:**

1. Вычисляем `hashCode()` ключа → применяем `hash = h ^ (h >>> 16)` для улучшения распределения.
2. `index = (capacity - 1) & hash` → находим номер бакета в массиве `table`.
3. В бакете хранится `LinkedList` (цепочка `Node`: `hash`, `key`, `value`, `next`).
4. Если цепочка в бакете вырастает до > 8 элементов (`TREEIFY_THRESHOLD`) → цепочка преобразуется в `TreeNode` (красно-чёрное дерево), поиск становится O(log n) вместо O(n).
5. Если `size > capacity * loadFactor` (по умолчанию 0.75) → `resize()`: массив удваивается, все элементы перераспределяются (O(n)).

**Обратное преобразование**: дерево → список при ≤ 6 элементов в бакете (`UNTREEIFY_THRESHOLD`).

- **Структура**: Хеш-таблица, основанная на массиве бакетов (`Node<K,V>[] table`).
- **Хеширование**: `hash = key.hashCode() ^ (key.hashCode() >>> 16)` — XOR со старшими битами снижает коллизии при малой capacity.
