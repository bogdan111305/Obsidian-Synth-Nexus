...
> [!QUOTE] Суть
> **Set** — коллекция уникальных элементов. `HashSet`: O(1) add/contains/remove, no order (внутри — HashMap). `LinkedHashSet`: порядок вставки + O(1). `TreeSet`: O(log n), сортировка. Уникальность определяется через `equals()` + `hashCode()` — **оба должны быть переопределены!**

> [!WARNING] Ловушка: Set и отсутствие equals/hashCode
> `new HashSet<>()` с объектами, у которых не переопределены `equals`/`hashCode`, считает все объекты уникальными (сравнение по ссылке). Множество будет содержать дублирующиеся по смыслу объекты.
