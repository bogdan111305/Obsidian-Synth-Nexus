...
## 2. Как компилятор реализует `enum`?
...
## 3. Базовый класс `java.lang.Enum`
...
public class DayCounter {
    private static final EnumMap<Day, Integer> counts = new EnumMap<>(Day.class);
    private static final ConcurrentHashMap<Day, Integer> concurrentCounts = new ConcurrentHashMap<>();

    public static synchronized void increment(Day day) {
        counts.compute(day, (k, v) -> v == null ? 1 : v + 1);
    }

    public static void incrementConcurrent(Day day) {
        concurrentCounts.compute(day, (k, v) -> v == null ? 1 : v + 1);
    }
}

new Thread(() -> DayCounter.incrementConcurrent(Day.MONDAY)).start();
```
...
- **Создание**: Константы создаются один раз при загрузке класса (O(1) на константу).
- **Доступ**: O(1) для получения константы или вызова методов.
- **Коллекции**:
  - `EnumSet`: Битовая карта, O(1) для операций.
  - `EnumMap`: Массив, O(1) доступ по ключу.
- **`values()`**: O(n) для создания копии массива.
- **`valueOf`**: O(n) из-за рефлексии.

**Рекомендация**:
- Используйте `EnumSet`/`EnumMap` вместо `HashSet`/`HashMap` для перечислений.
- Минимизируйте вызовы `valueOf` в горячих участках кода.

## 12. Многопоточность

Перечисления потокобезопасны по умолчанию:
- **Иммутабельность**: Константы и их поля (если `final`) неизменяемы.
- **Синглтон**: Создаются один раз при загрузке класса.
- **Потокобезопасные коллекции**:
  ```java
  import java.util.concurrent.ConcurrentHashMap;

  public class DayCounter {
      private static final ConcurrentHashMap<Day, Integer> counts = new ConcurrentHashMap<>();

      public static void increment(Day day) {
          counts.compute(day, (k, v) -> v == null ? 1 : v + 1);
      }
  }
  ```
...
```java
new Thread(() -> DayCounter.increment(Day.MONDAY)).start();
new Thread(() -> DayCounter.increment(Day.MONDAY)).start();
```

**Подводные камни**:
- Если `enum` содержит изменяемые поля, нужна синхронизация:
  ```java
  public enum Counter {
      INSTANCE;
      private int count;
...
## 13. Современные возможности

### 13.1. Pattern Matching (Java 17+)
```java
...
EnumSet.allOf(Day.class)
       .stream()
       .map(Day::name)
       .forEach(System.out::println); // MONDAY, TUESDAY, WEDNESDAY
```

### 13.3. Sealed-классы
Перечисления могут использоваться с sealed-классами для ограничения типов.

**Пример**:
```java
public sealed interface Event permits WorkEvent {
    Day getDay();
}

public record WorkEvent(Day day) implements Event {
    public WorkEvent {
        if (day != Day.MONDAY && day != Day.TUESDAY && day != Day.WEDNESDAY) {
            throw new IllegalArgumentException("Not a workday");
        }
    }
}
```
...
1. **Используйте для фиксированных наборов**:
   ```java
   public enum Status { ACTIVE, INACTIVE, DELETED }
   ```
2. **Добавляйте методы и поля**:
   ```java
   public enum Color {
       RED("#FF0000");
       private final String hexCode;
   ...
   ```
3. **Используйте `EnumSet` и `EnumMap`**:
   ```java
   EnumSet<Day> workdays = EnumSet.of(Day.MONDAY, Day.TUESDAY);
   ```
4. **Избегайте `ordinal`**:
   - Вместо `ordinal` используйте поля:
     ```java
...
1. **Зависимость от `ordinal`**:
   - Изменение порядка констант ломает логику.
   - **Решение**: Используйте поля вместо `ordinal`.
2. **Сериализация**:
   - Перечисления автоматически сериализуемы, но кастомные поля требуют внимания.
   - **Решение**: Реализуйте `readObject`/`writeObject` для сложных полей:
     ```java
     ...
     ```
3. **Сложные перечисления**:
   - Много методов и полей усложняют читаемость.
   - **Решение**: Разделяйте сложную логику на классы/интерфейсы.
...
public enum Day {
    ...
    public static Day fromRussianName(String russianName) {
        return Arrays.stream(values())
                     .filter(day -> day.russianName.equals(russianName))
                     .findFirst()
                     .orElseThrow(() -> new IllegalArgumentException("Unknown day: " + russianName));
    }
}

public class EnumExample {
    private static final Logger LOGGER = Logger.getLogger(EnumExample.class.getName());
    private static final ConcurrentHashMap<Day, Integer> counts = new ConcurrentHashMap<>();
    private static final EnumMap<Day, Integer> hoursWorked = new EnumMap<>(Day.class);

    public static void main(String[] args) {
        // Основное использование
        Day day = Day.MONDAY;
        System.out.println(day.getRussianName()); // Понедельник
        ...
        System.out.println(desc); // Рабочий день

        // EnumSet
        EnumSet<Day> workdays = EnumSet.of(Day.MONDAY, Day.TUESDAY, Day.WEDNESDAY);
        System.out.println("Рабочие дни: " + workdays); // [MONDAY, TUESDAY, WEDNESDAY]

        // EnumMap
        EnumMap<Day, String> descriptions = new EnumMap<>(Day.class);
        descriptions.put(Day.MONDAY, "Начало недели");
        descriptions.put(Day.TUESDAY, "Второй день");
        System.out.println("Описание: " + descriptions.get(Day.MONDAY)); // Начало недели
        ...
        // Многопоточность
        new Thread(() -> {
            counts.compute(Day.MONDAY, (k, v) -> v == null ? 1 : v + 1);
            LOGGER.info("Счётчик: " + counts.get(Day.MONDAY));
        }).start();
        ...
        EnumSet.allOf(Day.class)
               ...
               .map(Day::getRussianName)
               .forEach(System.out::println); // Понедельник, Вторник, Среда, Четверг, Пятница

        // Pattern matching
        Object obj = Day.MONDAY;
        if (obj instanceof Day d) {
            System.out.println("Pattern matching: " + d.getRussianName());
        }
    }
}
```
...
true
Рабочий день
Рабочие дни: [MONDAY, TUESDAY, WEDNESDAY]
Описание: Начало недели
Всего часов: 15
Понедельник
Вторник
Среда
