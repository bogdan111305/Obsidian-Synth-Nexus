...
> [!QUOTE] Суть
> `Object` — корень иерархии всех классов Java. Каждый класс неявно наследует `Object` и получает методы `toString()`, `equals()`, `hashCode()`, `clone()`, `getClass()`, `wait()`/`notify()`. Правильное переопределение `equals` и `hashCode` критично для работы коллекций.
...
> [!WARNING] Ловушка: контракт equals/hashCode
> Если переопределяешь `equals` — **обязан** переопределить `hashCode`. Иначе объект нельзя будет найти в `HashMap`/`HashSet`: `map.get(key)` вернёт `null` даже если ключ добавлен. Используй `Objects.hash(field1, field2)` для генерации.
...
> [!WARNING] Ловушка: finalize() — никогда не используй
> `finalize()` — устарел в Java 9, удалён в Java 18. **Нет гарантии вызова**: GC может не вызвать его никогда. Замедляет GC и создаёт риск воскрешения объекта. Используй `try-with-resources` или `java.lang.ref.Cleaner`.
