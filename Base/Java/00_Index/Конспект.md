...
- [ ] `invokespecial` vs `invokevirtual`: почему `super.method()` не рекурсивен и как JVM это обеспечивает?
...
| [[Наследование]] | Core→Deep | `extends`, `super`, `invokespecial`, bridge methods, constructor trap |
| [[Полиморфизм]] | Core→Deep | vtable dispatch, inline cache, megamorphic deopt |
...
| [[Java Exceptions]] | Core→Deep | Иерархия, `try-with-resources` bytecode, `fillInStackTrace` perf, FastException |
...
- [ ] **Capturing vs non-capturing лямбда**: почему `() -> 42` — singleton, а `() -> x` — новый объект при каждом вызове?
- [ ] `effectively final` в `parallelStream()`: почему `int[] counter = {0}` создаёт data race?
...
| [[Функциональное программирование с коллекциями]] | Core→Deep | `Predicate`, `Function`, `Optional`, invokedynamic, capturing/non-capturing lambda |
...
- [ ] **AQS CLH queue**: что такое `Node.waitStatus` (SIGNAL/CANCELLED/CONDITION/PROPAGATE)? Как `acquireQueued` CAS-loop паркует поток?
- [ ] `isInterrupted()` vs `Thread.interrupted()`: в чём разница и что такое «кража флага» (flag theft)?
- [ ] `interrupt()` + NIO: почему прерывание потока закрывает `InterruptibleChannel`?
...
| [[Прерывание потока в Java]] | Core→Deep | `interrupt()`, `isInterrupted()` flag theft, NIO channel close, virtual threads |
| [[Atomic]] | Core→Deep | `AtomicInteger`, `LongAdder` / Striped64, VarHandle ordering modes |
...
[[Примитивные типы данных]] · [[Java Generic]] · [[Общая иерархия коллекций]] · [[Интерфейс Map]] · [[Java Stream API]] · [[Процессы и Потоки, Thread, Runnable, состояния потоков|Потоки]] · [[Synchronized]] · [[Атомарность операций и Volatile]] · [[ThreadPool, Future, Callable, Executors, CompletableFuture|CompletableFuture]] · [[Java Memory Structure]] · [[ClassLoaders]] · [[Современные возможности Java]]

### Deep Dive — от Middle к Senior
[[Наследование]] · [[Полиморфизм]] · [[Java Exceptions]] · [[Функциональное программирование с коллекциями]] · [[Java Reflection API]] · [[Производительность коллекций]] · [[Прерывание потока в Java]] · [[Atomic]] · [[CAS и Unsafe]] · [[Lock]] · [[Модель памяти Java (JMM) и барьеры памяти]] · [[JIT Compiler & Optimizations]] · [[JVM Profiling & Observability]] · [[NIO Networking]] · [[Java Serialization and Deserialization]]
